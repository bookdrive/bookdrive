class SnippetsController < ApplicationController
  ...
  # GET /snippets.xml
  def index
    @snippets = Snippet.search(params[:search]).order(sort_column + " " + sort_direction).paginate(:per_page => 25, :page => params[:page])
  end
  ...
  def sort_column
    Snippet.column_names.include?(params[:sort]) ? params[:sort] : "name"
  end
